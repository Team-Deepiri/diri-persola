...
class CityService:
    ...
    def _serialize_member(self, member: FamilyMemberModel) -> dict[str, Any]:
        ...
        return {
            ...
            "max_age_ticks": int(
                getattr(member, "max_age_ticks", DEFAULT_MAX_AGE_TICKS) or DEFAULT_MAX_AGE_TICKS
            ),
            "life_status": getattr(member, "life_status", None)
            or FamilyMemberLifeStatus.ALIVE.value,
            ...
            "deceased_at": member.deceased_at.isoformat()
            if getattr(member, "deceased_at", None)
            else None,
            "successor_of_id": str(member.successor_of_id)
            if getattr(member, "successor_of_id", None)
            else None,
    ...
    def _serialize_family(
        self, family: FamilyModel, members: list[FamilyMemberModel] | None = None
    ) -> dict[str, Any]:
    ...
    def _serialize_run(row: WorkspaceRunModel) -> dict[str, Any]:
        return {
            ...
            "started_by_agent_id": str(row.started_by_agent_id)
            if row.started_by_agent_id
            else None,
    ...
    async def spawn_child(
        ...
        *,
        ...
        overrides = {
            k: _clamp_knob(v) for k, v in (knob_overrides or {}).items() if k in PERSONA_KNOB_FIELDS
        }
        inherited_tags = (
            list(tool_tags)
            if tool_tags is not None
            else list(parent_member.tool_tags or DEFAULT_CITY_TOOL_TAGS)
        )
        ...
        child_member = await self.members.create(
            FamilyMemberModel(
                ...
                age_ticks=0,
                max_age_ticks=int(
                    (family.policy or {}).get(
                        "max_age_ticks",
                        getattr(parent_member, "max_age_ticks", DEFAULT_MAX_AGE_TICKS),
                    )
                ),
                life_status=FamilyMemberLifeStatus.ALIVE.value,
                goals=default_goals(role_label),
            ...
            generations.append(
                {
                    ...
                    "avg_structured_thinking": round(b["thinking_sum"] / living, 4)
                    if living
                    else 0.0,
                    # Productivity proxy: thinking × (1 + growth) among the living
                    "productivity_index": round(
                        (b["thinking_sum"] / living) * (1.0 + (b["growth_sum"] / living))
                        if living
                        else 0.0,
                        4,
                    ),
    ...
    async def update_family_policy(
        self, family_id: UUID, policy_patch: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def record_run(
        ...
        *,
        ...
        await self.emit_event(
            ...
            if status != WorkspaceRunStatus.PENDING.value
            else "run.started",
            family_id=job.family_id,
            job_id=job_id,
            payload={
        ...
        try:
            ...
            for call in calls:
                if isinstance(call, str):
                    ...
                    normalized.append(
                        {"name": str(name), "args": args if isinstance(args, dict) else {}}
                    )
        ...
        *,
        ...
        async def _as(role: str, calls: list[dict[str, Any]]) -> None:
            ...
            contributions.append(
                {"role": role, "agent_id": str(agent_id) if agent_id else None, **result}
            )
            ...
            [
                ...
                },
                {
                    ...
                    "args": {
                        ...
                        "payload": {"from": "creative", "mood": "excited"},
                    },
                },
            ],
        )
            ...
            [
                {
                    ...
                    "args": {
                        "path": "hello.py",
                        "content": ('print("persola-city-wedge")\nprint("sum", 1 + 2 + 3)\n'),
                    },
                },
        ...
        succeeded = any(
            r.get("status") == "succeeded" and r.get("tool") == "run_python" for r in runs
        )
    ...
    async def city_pulse(
        ...
        *,
        ...
        avg_cohesion = (
            round(sum(r["cohesion"]["score"] for r in results) / len(results), 4)
            if results
            else 0.0
        )
        avg_contributors = (
            round(sum(r["contributor_count"] for r in results) / len(results), 2)
            if results
            else 0.0
        )
    ...
    async def _succeed_member(
        self, deceased: FamilyMemberModel, family: FamilyModel
    ) -> FamilyMemberModel:
        ...
        if (
            living_parent is not None
            and living_parent.life_status == FamilyMemberLifeStatus.ALIVE.value
        ):
        ...
        thinking = min(
            1.0,
            float(deceased.structured_thinking or 0.5) * 0.85
            + float(deceased.growth or 0.0) * 0.2
            + 0.05,
        )
        ...
        role_in_family = (
            FamilyMemberRole.PARENT.value if was_parent else FamilyMemberRole.CHILD.value
        )
        ...
        heir = await self.members.create(
            FamilyMemberModel(
                ...
                max_age_ticks=int(
                    (family.policy or {}).get(
                        "max_age_ticks", deceased.max_age_ticks or DEFAULT_MAX_AGE_TICKS
                    )
                ),
                life_status=FamilyMemberLifeStatus.ALIVE.value,
                goals=goals,
        ...
        *,
        ...
        for s in summaries:
            ...
            alive_before = [
                ...
                for m in family.members
                if m.is_active
                and (m.life_status or FamilyMemberLifeStatus.ALIVE.value)
                == FamilyMemberLifeStatus.ALIVE.value
            ]
    ...
    async def scale_probe(
        ...
        *,
        ...
        """
        from ..orchestration.city_personalities import (
        ...
        for i in range(n_families):
            ...
            if run_jobs:
                ...
                if agent_id is None:
                    child = next(
                        (m for m in detail["members"] if m["role_in_family"] == "child"), None
                    )
    ...
    async def bulk_cyrex_sync(
        ...
        *,
        ...
        for m in family["members"]:
            if (
                living_only
                and (m.get("life_status") or "alive") != FamilyMemberLifeStatus.ALIVE.value
            ):
                skipped += 1
                continue
    ...
    async def invoke_team_on_job(
        ...
        *,
        ...
        payload["city_tools"] = [
            t["name"] for t in registry.list_tools() if "city" in (t.get("tags") or [])
        ]
        await self.db.commit()
        return payload

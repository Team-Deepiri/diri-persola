...
def _score_constraints(*field_names: str) -> list[CheckConstraint]:
    return [
        CheckConstraint(
            f"{field_name} >= 0.0 AND {field_name} <= 1.0", name=f"ck_{field_name}_range"
        )
...
class PersonaModel(UUIDPrimaryKeyMixin, UpdatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        *_score_constraints(*PERSONA_KNOB_FIELDS),
        CheckConstraint(
            "temperature >= 0.0 AND temperature <= 2.0", name="ck_personas_temperature_range"
        ),
        CheckConstraint(
            "max_tokens >= 1 AND max_tokens <= 32000", name="ck_personas_max_tokens_range"
        ),
    )
...
class AgentModel(UUIDPrimaryKeyMixin, UpdatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_agents_temperature_range",
        ),
        CheckConstraint(
            "max_tokens IS NULL OR (max_tokens >= 1 AND max_tokens <= 32000)",
            name="ck_agents_max_tokens_range",
        ),
        _enum_constraint(
            "role", tuple(role.value for role in AgentRole), name="ck_agents_role_values"
        ),
    )

    name: Mapped[str] = mapped_column(String(255), nullable=False)
    role: Mapped[str] = mapped_column(
        String(100), nullable=False, default=AgentRole.ASSISTANT.value
    )
    ...
    tool_configs: Mapped[list[AgentToolModel]] = relationship(
...
class MessageModel(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        _enum_constraint(
            "role", tuple(role.value for role in MessageRole), name="ck_messages_role_values"
        ),
    )
...
class PersonaVersionModel(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        _enum_constraint(
            "source",
            tuple(source.value for source in PersonaVersionSource),
            name="ck_persona_version_source_values",
        ),
    )
    ...
    source: Mapped[str] = mapped_column(
        String(50), nullable=False, default=PersonaVersionSource.MANUAL.value
    )
...
class AgentToolModel(UUIDPrimaryKeyMixin, UpdatedAtMixin, Base):
    ...
    tool_config: Mapped[dict[str, Any]] = mapped_column(
        "config", JSONB, nullable=False, default=dict
    )
...
class AgentRunModel(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        _enum_constraint(
            "status",
            tuple(status.value for status in AgentRunStatus),
    ...
    status: Mapped[str] = mapped_column(
        String(30), nullable=False, default=AgentRunStatus.PENDING.value
    )
    provider: Mapped[str | None] = mapped_column(String(50), nullable=True)
    model: Mapped[str | None] = mapped_column(String(100), nullable=True)
...
class TeamSessionModel(UUIDPrimaryKeyMixin, UpdatedAtMixin, Base):
    ...
    workflows: Mapped[list[TeamWorkflowModel]] = relationship(
        back_populates="team_session", cascade="all, delete-orphan"
    )
    memory_entries: Mapped[list[TeamMemoryModel]] = relationship(
        back_populates="team_session", cascade="all, delete-orphan"
    )
...
class TeamWorkflowModel(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        _enum_constraint(
            "status",
            tuple(s.value for s in TeamWorkflowStatus),
            name="ck_team_workflows_status_values",
        ),
    )
    ...
    status: Mapped[str] = mapped_column(
        String(30), nullable=False, default=TeamWorkflowStatus.PENDING.value
    )
    ...
    steps: Mapped[list[TeamWorkflowStepModel]] = relationship(
...
class FamilyModel(UUIDPrimaryKeyMixin, UpdatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        _enum_constraint(
            "default_district",
            tuple(d.value for d in CityDistrict),
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    default_district: Mapped[str] = mapped_column(
        String(30), nullable=False, default=CityDistrict.BUILD.value
    )
    ...
    jobs: Mapped[list[CityJobModel]] = relationship(
    ...
    events: Mapped[list[CityEventModel]] = relationship(
        back_populates="family", cascade="all, delete-orphan"
    )
...
class FamilyMemberModel(UUIDPrimaryKeyMixin, UpdatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        _enum_constraint(
            "role_in_family",
            tuple(r.value for r in FamilyMemberRole),
    ...
    role_in_family: Mapped[str] = mapped_column(
        String(20), nullable=False, default=FamilyMemberRole.CHILD.value
    )
...
class CityJobModel(UUIDPrimaryKeyMixin, UpdatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        _enum_constraint(
            "district", tuple(d.value for d in CityDistrict), name="ck_city_jobs_district"
        ),
        _enum_constraint(
            "status", tuple(s.value for s in CityJobStatus), name="ck_city_jobs_status"
        ),
    )
    ...
    status: Mapped[str] = mapped_column(
        String(30), nullable=False, default=CityJobStatus.PENDING.value
    )
    ...
    runs: Mapped[list[WorkspaceRunModel]] = relationship(
...
class WorkspaceRunModel(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        _enum_constraint(
            "status", tuple(s.value for s in WorkspaceRunStatus), name="ck_workspace_runs_status"
        ),
    )
    ...
    status: Mapped[str] = mapped_column(
        String(30), nullable=False, default=WorkspaceRunStatus.PENDING.value
    )

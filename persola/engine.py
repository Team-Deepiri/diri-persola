...
from .models import DEFAULT_PRESETS, KNOB_DEFINITIONS, PersonaProfile, PresetName
...
Always respond according to these parameters while being helpful and accurate."""
    ...
    def blend_personas(
        self, profile1: PersonaProfile, profile2: PersonaProfile, ratio: float = 0.5
    ) -> PersonaProfile:
    ...
    def blend_multiple(
        self, profiles: list[PersonaProfile], weights: list[float]
    ) -> PersonaProfile:
        """
        ...
        return PersonaProfile(
            ...
            **{
                **blended_knobs,
                ...
                "temperature": profiles[0].temperature,
                "max_tokens": profiles[0].max_tokens,
            },
        )
        ...
        for knob in self.knob_definitions:
            if knob.key in knobs:
                ...
                if not (knob.min_value <= value <= knob.max_value):
                    valid = False
                    errors.append(
                        f"{knob.key} must be between {knob.min_value} and {knob.max_value}"
                    )

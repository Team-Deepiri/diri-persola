...
from persola.orchestration.city_pulse import DISTRICT_ROLE_PREFERENCE, district_tool_calls, pick_agent_for_district
...
class TestCityPulseService:
	...
	async def test_cohesion_veto(self, db_session):
		...
		job = await service.start_job(family_id=UUID(family["id"]), goal="veto-me", district="build")
...
class TestCityPulseAPI:
	async def test_pulse_endpoint(self, http_client):
		...
		pulse = await http_client.post("/api/v1/city/pulse", json={"max_families": 2, "auto_merge": True})

...
from persola.orchestration.city_scale import (
    ConcurrencyGovernor,
    ModelTier,
    ScaleConfig,
    shard_for_district,
...
class TestScalePrimitives:
    ...
    async def test_governor_limits_per_family(self):
        gov = ConcurrencyGovernor(
            ScaleConfig(max_global_concurrent=4, max_per_family=1, max_per_district=4)
        )
...
class TestScaleProbe:
    ...
    async def test_cohesion_score(self, db_session):
        ...
        probe = await service.scale_probe(
            families=1, agents_per_family=3, name_prefix="Coh", run_jobs=True
        )

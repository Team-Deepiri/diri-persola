    ...
    def test_child_fingerprints_are_unique_across_hundred(self):
        ...
        for fam in range(10):
            ...
            for child in range(9):
                fps.add(
                    distinct_child_personality(child_index=child, family_index=fam)["fingerprint"]
                )
        assert len(fps) == 100
...
class TestHundredAwaken:
    ...
    async def test_city_snapshot(self, db_session):
        service = CityService(db_session)
        await service.scale_probe(
            families=2, agents_per_family=3, name_prefix="Snap", run_jobs=False
        )

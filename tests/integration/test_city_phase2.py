...
from persola.orchestration.sandbox import SandboxError, run_python_sandboxed, sanitize_workspace_path
...
class TestCityBuildRunAPI:
		...
		inv = await http_client.post(
			...
			json={
				...
				"calls": [
					...
					{"name": "emit_viz_event", "args": {"event_type": "viz.pulse", "payload": {"note": "done"}}},
				],
			},
		)
	...
	async def test_list_city_tools(self, http_client):
		...
		assert {"workspace_write", "workspace_read", "workspace_list", "run_python", "emit_viz_event"} <= names
...
class TestOrchestratorParsesToolCalls:
	async def test_tool_runner_prefers_structured_calls(self):
		...
		assert results[0]["name"] == "echo" or results[0].get("result", {}).get("echo") == "from-executor"
